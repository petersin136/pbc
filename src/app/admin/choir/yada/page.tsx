...
export default function YadaChoirAdminPage() {
  const [loading, setLoading] = useState(true);
  const [sections, setSections] = useState<Section[]>([]);
  const [showAddModal, setShowAddModal] = useState(false);
  const [editingSection, setEditingSection] = useState<Section | null>(null);
  const PAGE_ID = "choir-yada";
  ...
  return (
    <div className="max-w-5xl mx-auto">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900 mb-4">🎵 야다 성가대 관리</h1>
        <p className="text-gray-600 mb-4">
          야다 성가대 페이지의 콘텐츠를 관리합니다.
        </p>
      ...
      {sections.length === 0 ? (
        <EmptyState
          icon="🎵"
          title="섹션이 없습니다"
          description="야다 성가대 페이지에 아직 섹션이 없습니다."
          actionLabel="섹션 추가"
          onAction={handleAdd}
        />
      ) : (
